...
class DVCFileMissingError(FileNotFoundError):
    ...
    def __init__(self, repo: str, file_path: str):
        ...
        super().__init__(
            f"Missing DVC file {self.file_path} in repo {self.repo}"
        )
...
class DVCCliCommandError(Exception):
    ...
    def __init__(
        ...
        super().__init__(
            f"DVC Command {self.dvc_command} failed with status code {self.dvc_exit_code} and output: {self.dvc_output}"
        )
...
class DVCMissingExecutableError(Exception):
    ...
    def __init__(self):
        super().__init__(
            "DVC Python library is missing and DVC "
...
class DVCGitRepoNotAccessibleError(Exception):
    ...
    def __init__(self, repo: str, git_exception: exc.GitError):
        ...
        self.repo = repo
        super().__init__(
            f"Could not clone git repository: {repo}. Error: {git_exception}"
        )
...
class DVCGitUpdateError(Exception):
    ...
    def __init__(
        self, repo: str, updated_files: List[str], git_exception: exc.GitError
    ):
        self.git_exception = git_exception
        self.updated_files = updated_files
        self.repo = repo
        super().__init__(
            f"Cannot update DVC. Git push failed for repository "
...
class DVCInvalidVersion(Exception):
    ...
    def __init__(
        self, description: str, version: Version, constraint: SimpleSpec
    ):
        self.version = version
        self.description = description
        self.constraint = constraint
        super().__init__(
            f"{self.description}. Required version: {constraint}. Got: {version}"
        )
        ...
        try:
            ...
            if not disable_error_message:
                LOGS.exceptions.error(
                    f"Error was thrown inside the dvc-fs code. "
